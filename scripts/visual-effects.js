...
(function exposeVisualEffects(global) {
  ...
  class VisualEffectsSystem {
    constructor(){
      ...
      this.atlas=null;this.productionAtlas=null;this.referenceAtlas=null;this.assetsReady=false;this.productionReady=false;this.referenceReady=false;this.assetsFailed=false;this._regionCache=new Map();this._preloadWildwood();this._preloadProduction();this._preloadReference();
    }
    _preloadWildwood(){
    ...
    drawBattleActor(ctx,zone,kind,id,cx,groundY,maxH){if(!this.isWildwood(zone)||!this.productionReady)return false;const visual={skirmisher:'rogue',invoker:'arcanist',occultist:'acolyte'}[id]||id;const name=kind==='player'?`class-${visual||'vanguard'}`:this.enemyRegionFor(id),r=name&&this.productionRegion(name);if(!r)return false;const h=maxH*1.1,w=h*r[2]/r[3],x=cx-w/2,y=groundY-h;ctx.save();ctx.imageSmoothingEnabled=true;const separation=ctx.createRadialGradient(cx,groundY-h*.48,8,cx,groundY-h*.48,w*.9);separation.addColorStop(0,'rgba(2,5,4,.62)');separation.addColorStop(.7,'rgba(2,5,4,.26)');separation.addColorStop(1,'rgba(2,5,4,0)');ctx.fillStyle=separation;ctx.fillRect(cx-w,groundY-h*1.15,w*2,h*1.25);ctx.globalAlpha=.58;ctx.fillStyle='#010201';ctx.beginPath();ctx.ellipse(cx,groundY-2,w*.39,7,0,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;ctx.filter=kind==='player'?'drop-shadow(-2px -1px 3px rgba(115,181,210,.65)) drop-shadow(2px 0 4px rgba(239,185,104,.42))':'drop-shadow(-2px -1px 4px rgba(174,122,231,.68)) drop-shadow(2px 1px 5px rgba(239,177,105,.3))';const ok=this.drawProductionRegion(ctx,name,x,y,w,h,true);ctx.restore();return ok?{sx:x,sy:y,w,h}:false;}
    ...
    drawWildwoodGround(ctx,w,h,zone,map,camX,camY,tileSize,frame){
      ...
      for(let ty=startY;ty<=endY;ty++)for(let tx=startX;tx<=endX;tx++){
        ...
        ctx.globalAlpha=deep?.7:.82;this.drawRegion(ctx,name,x-2,y-2,tileSize+4,tileSize+4);
        // Soft overlapping edge wash removes the contact-sheet crop's square read.
        ctx.globalAlpha=.28;ctx.fillStyle=id>=41&&id<=64?'#59472f':deep?'#151b12':'#343622';ctx.beginPath();ctx.ellipse(x+tileSize*(.35+seed*.3),y+tileSize*.55,tileSize*.74,tileSize*.56,seed*2,0,Math.PI*2);ctx.fill();
      }
    ...
    drawWildwoodActor(ctx,zone,kind,facing,worldX,worldY,camX,camY,tileSize){
      ...
      const heroFacing={down:'hero-down',up:'hero-up',left:'hero-left',right:'hero-right'};if(kind==='player'&&this.productionReady){const heroName=heroFacing[facing]||'hero-down',heroRegion=this.productionRegion(heroName),h=76,w=h*heroRegion[2]/heroRegion[3],x=worldX*tileSize-camX+tileSize/2-w/2,y=(worldY+1)*tileSize-camY-h;ctx.save();ctx.imageSmoothingEnabled=true;const halo=ctx.createRadialGradient(x+w/2,y+h*.58,4,x+w/2,y+h*.58,38);halo.addColorStop(0,'rgba(9,8,5,.48)');halo.addColorStop(1,'rgba(9,8,5,0)');ctx.fillStyle=halo;ctx.fillRect(x-20,y-10,w+40,h+20);ctx.globalAlpha=.62;ctx.fillStyle='#020302';ctx.beginPath();ctx.ellipse(x+w/2,y+h*.96,w*.42,5,0,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;ctx.filter='drop-shadow(-1px -1px 2px rgba(112,184,203,.62)) drop-shadow(2px 0 3px rgba(239,181,91,.48))';const ok=this.drawProductionRegion(ctx,heroName,x,y,w,h,true);ctx.restore();return ok;}
